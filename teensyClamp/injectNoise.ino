...
bool nextCalculated;
float scaleFactor;
uint32_t numFloatsToSend = floatsPerStandardDataTransfer*ceil((4+2*numSteps)/floatsPerStandardDataTransfer);    //round up to floatsPerStandardDataTransfer
...
FASTRUN void injectNoise() {
  ...
  noise_dt = 0; //to track how long cross corr took
//  memset(crossCorr,0,sizeof(crossCorr)); //cross corr output must be initialized to zero, so use memset do that
//  arm_correlate_f32(injected,numSteps,memVoltage,numSteps,crossCorr);
  //stepOvershoot = noise_dt; //reusing this variable for storage
  //transfer results to Igor -- need to send in 16s
  uint32_t sendCount = 4;
  sendFloat(sendCount);   //indicate position at which data starts. this is pos 0
  sendFloat(badStepCount);    //1
  sendFloat(worstStepOvershoot); //2 
  sendFloat(numSteps);    //3
  for (size_t i=0;i<numSteps;i++){
    sendFloat(injected[i]);
    sendCount++;
  }
   for (size_t i=0;i<numSteps;i++){
    sendFloat(memVoltage[i]);
    sendCount++;
  }
  for (size_t i=sendCount;i<numFloatsToSend;i++) {
    sendFloat(nanVal);
  }
}
